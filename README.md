...
Приложение для корректировки часов техобслуживания промышленного оборудования, подключенного через Siemens S7 ПЛК. Создано на Python Tkinter с удобным пользовательским интерфейсом для чтения и записи значений времени техобслуживания напрямую в блоки данных ПЛК.
...
- Проверка ввода с ограничением от 0 до 50 000 часов
...
- Проверка ввода (лимит 0-50 000 часов)
...
3. **Запись часов техобслуживания:**
   - Введите новое значение в поле "Часы" (0-50 000)
...
├── utils/
│   ├── ecs7tags2equips_mdb.py    # Конвертер тегов ECS7 из MDB
│   ├── ecs7tags2equips_sqlite.py # Конвертер тегов ECS7 из SQLite
│   ├── ecs8tags2equips.py        # Конвертер тегов ECS8
│   ├── wincc_tags2equips.py      # Конвертер тегов WinCC
│   ├── merge_equips.py           # Объединение equips файлов
...
- `ecs7tags2equips_mdb.py`: Конвертация тегов ECS7 из базы данных MDB (MS Access)
- `ecs7tags2equips_sqlite.py`: Конвертация тегов ECS7 из базы данных SQLite
- `ecs8tags2equips.py`: Конвертация тегов ECS8 из CSV
- `wincc_tags2equips.py`: Конвертация тегов WinCC
- `merge_equips.py`: Объединение нескольких equips файлов с удалением дубликатов по `eq_name`
...
- **v1.3.0** (2026-07-17) - Текущая версия
  - Увеличен размер окна до 1280×800
  - Увеличен шрифт таблицы (14px) и высота строк
  - Добавлена цветовая маркировка строк по ПЛК (990–997)
  - Удалены дубликаты записей в equips.json (581 уникальных)
  - Добавлена утилита конвертации ECS8 (Points.xlsx → equips2.json)
